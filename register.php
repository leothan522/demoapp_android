...
function registrar($name, $email, $telefono, $password, $fcm_token){
    $query = new Query();
    $row = null;
    ...
    $sql = "INSERT INTO `users` (`name`, `email`, `password`, `telefono`, `fcm_token`, `created_at`, `updated_at`) 
            VALUES ('{$name}', '{$email}', '{$password}', '{$telefono}', '{$fcm_token}', '{$date}', '{$date}');";
    $query->save($sql);
...
if (empty($_POST['name']) || empty($_POST['email']) || empty($_POST['telefono']) || empty($_POST['password'])) {
    ...
    $usuario = registrar(ucwords($_POST['name']), $_POST['email'], $_POST['telefono'], $_POST['password'], $_POST['fcm_token']);
    if ($usuario){
        $data['id'] = $usuario['id'];
